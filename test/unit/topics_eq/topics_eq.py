...
def check_topic_lines(topic_eq_lines):
...
def find_next_call(call_num, call_lines):
    take_next = False
    next_call = None
...
def int_or_space(number):
    if number == '':
        return ''
    else:
        int(number)

def lookup_topic_eq(call, topic, ref_call, call_lines, topic_conv_lines):
    ...
    for topic_conv_line in topic_conv_lines:
        ...
        if line_cur_call == call and line_cur_topic == topic:
            ...
            if eq_call == ref_call:
                ...
                topic_lookup_results = lookup_topic_eq(eq_call, eq_topic, ref_call, call_lines, topic_conv_lines)
...
def main():
    ...
    # Open the CSV file
    call_lines = read_csv('../../test_data/topics_eq/calls_test.csv')
    topic_conv_lines = read_csv('../../test_data/topics_eq/topics_eq_test3.csv')

    if check_topic_lines(topic_conv_lines):
        # Display the list
        # print(call_lines)
        # print(call_topics)
        ...
        call_num, topic_num = split_topic_id(topic_id)

        if ref_call == call_num:
            print("Source call and goal call are equivalent = " + str(ref_call))
        else:
            ref_topics = lookup_topic_eq(call_num, topic_num, ref_call, call_lines, topic_conv_lines)
